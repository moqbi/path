...
لا شيء. آخر بندين أُنجزا:

1. **`src/app/music/page.tsx`** ← **أُلغيت**: ربط سبوتيفاي حُذف من الويب
   بقرار صاحب المشروع، ومحلّه رابطٌ يُلصق (`kind=MUSIC` مع `musicUrl`)
   يُقرأ بـoEmbed الرسميّ ليوتيوب وساوندكلاود وسبوتيفاي — بلا OAuth ولا
   مفاتيح. الشاشة في `apps/mobile/app/compose.tsx`.
2. **`src/app/admin/`** ← **نُقلت إلى `apps/web/src/app/(admin)/`** كما
   هي، ومعها ما تحتاجه من `lib` و`components` وإجراءات اللوحة وحدها في
   `apps/web/src/app/actions.ts`. وزِيد فيها قسما **البلاغات** و**الكلمات
   الممنوعة**. والويب الحالي (`src/`) يبقى يعمل على ريندر حتى Sprint 10،
   فالنسختان على قاعدةٍ واحدة — لا مخطّطَ ثانٍ ولا هجرةَ ثانية.
...
- **الصور تبقى فارغةً في معاينة الويب وحدها**: `react-native-web` يترجم
  ...
  هذا الخطأ.
- **الاتجاه يُفرض من الجذر، ولا يُكتب صفّاً صفّاً**: `app/_layout.tsx`
  يضبط `I18nManager` و`direction: "rtl"` على جذر الشجرة و`dir="rtl"` على
  مستند المعاينة. فكلّ صفٍّ يُكتب `row` كما يُكتب في الويب، وينقلب
  بنفسه. وما كان مكتوباً هنا من `row-reverse` في `tabBarStyle` لم يكن
  يعمل أصلاً: أصناف الشريط في حاويةٍ داخلية لا نملك نمطها.
- **كل حاوية مرنة تأخذ `minWidth: 0`** (القاعدة ١١ في `CLAUDE.md`):
  حقلُ إدخالٍ على الويب له عرضٌ ذاتيّ لا ينكمش تحته، فيخرج سطرُ الإرسال
  عن الشاشة — وفي RTL يخرج يساراً فيبدو التطبيق كلّه منزاحاً.
